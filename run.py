...
#6. 训练线性分类器(??输入数据的类型)
cmd = './ffm-train -k 6 -t 13 -s {nr_thread} -p {te_file_head}.ffm {tr_file_head}.ffm model'.format(nr_thread=NR_THREAD,tr_file_name=train_file_name,tr_file_head=train_file_head,te_file_head=test_file_head,te_file_name=test_file_name)
subprocess.call(cmd, shell=True)
